...
SCENARIO("A new channel", "[channel]")
{
   ...
   GIVEN("That no message has been sent over this channel yet")
   {
      ...
      {
         THEN("``true`` is returned") { REQUIRE(channel.empty()); }
      }
...
void
ping(Channel<std::string>& channel)
{
   for (;;) {
      channel << "ping!";
...
void
correspond(Channel<std::string>& inbox, Channel<std::string>& outbox)
{
   for (;;) {
      std::string message;
      inbox >> message;
...
void
block(Channel<std::string>& inbox, Channel<std::string>& outbox)
{
   std::string message;
